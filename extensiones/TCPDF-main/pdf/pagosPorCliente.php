...
if (!empty($idCliente)) {
    ...
    $respuestaPagos = ModeloPagos::mdlPagosPorCliente($idCliente);
    ...
    if (!empty($respuestaPagos)) {
        ...
        $html .= '<h4 style="text-align:center;">CI: ' . $documentoCliente . ' </h4>';
...
<tbody>';
        ...
        foreach ($respuestaPagos as $index => $pagos) {
            ...
            $mesNumero = date('m', strtotime($pagos['fecha']));
            $nombreMes = $mesesEspanol[$mesNumero];

            $html .= '<tr style="background-color: ' . $bgColor . ';">
        <td style="text-align:center; padding:5px;">' . $fechaFormateada . '</td>
        <td style="text-align:center; padding:5px;">' . $nombreMes . '</td>
